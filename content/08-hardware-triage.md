...
앞 절이 노드 안에서 GPU가 어디에 붙는지를 그렸다면, 이 절은 GPU 보드 한 장 안을 연다. 장애와
성능을 읽으려면 카드 안이 어떻게 나뉘어 도는지 알아야 한다.

GPU 연산의 기본 단위가 SM(Streaming Multiprocessor)이다. 카드 한 장에 SM 수십에서 백여 개가 들어
있고, 각 SM이 독립적으로 스레드 묶음을 돌린다. SM 안에는 연산기와 작은 공유 메모리, 레지스터가
함께 있다. GPU가 빠른 이유는 한 코어가 빨라서가 아니라 이런 SM이 많아 한꺼번에 도는 데 있다.

SM 안의 연산기는 종류가 갈린다.

| 연산기 | 맡는 연산 | 특징 |
| --- | --- | --- |
| CUDA Core | 일반 부동소수점·정수 연산 | 요소별 연산, 폭넓게 쓰인다 |
| Tensor Core | 행렬 곱 누적(작은 행렬 단위) | 딥러닝의 곱셈 누적을 몰아서 처리 |

CUDA Core는 값 하나하나에 대한 연산을 맡고, Tensor Core는 작은 행렬을 한 번에 곱해 더하는 연산에
특화됐다. 학습과 추론의 계산 대부분이 행렬 곱이라, 요즘 카드의 성능 수치는 Tensor Core에서 나온다.
같은 카드라도 Tensor Core를 쓰는 커널과 안 쓰는 커널의 처리량이 크게 벌어진다.

스케줄링 단위는 워프(warp)다. GPU는 스레드를 32개씩 묶은 워프 단위로 명령을 낸다. 한 워프의 32개
스레드가 같은 명령을 서로 다른 데이터에 적용한다. 그래서 워프 안의 스레드가 조건 분기로 서로 다른
길로 가면(분기 발산) 양쪽을 차례로 돌아 효율이 떨어진다. SM은 여러 워프를 쥐고 있다가, 한 워프가
메모리를 기다리는 동안 다른 워프를 올려 대기 시간을 숨긴다.

메모리는 HBM(High Bandwidth Memory) 스택으로 붙는다. HBM은 D램을 수직으로 쌓아 보드 위 GPU 옆에
바로 둔 메모리라 폭이 넓어 대역폭이 크다. GPU와 HBM 사이는 메모리 컨트롤러가 잇고, 이 대역폭이
학습 처리량을 좌우하는 경우가 많다. 연산기가 아무리 빨라도 HBM에서 데이터를 못 대면 SM이 논다.

세대별로 무엇이 달라졌는지 견줘 본다. 아래 수치는 SXM 기준의 대표값이다.

| 항목 | A100 | H100 | H200 |
| --- | --- | --- | --- |
| SM 수 | 108 | 132 | 132 |
| HBM 용량 | 80GB (HBM2e) | 80GB (HBM3) | 141GB (HBM3e) |
| HBM 대역폭 | 약 2.0TB/s | 약 3.35TB/s | 약 4.8TB/s |
| NVLink 세대 | 3세대 | 4세대 | 4세대 |
| 지원 정밀도 | FP64·TF32·FP16·BF16·INT8 | 위에 FP8 추가 | H100과 동일 |

H100부터 Transformer Engine이 들어갔다. Transformer Engine은 FP8이라는 8비트 부동소수점으로 행렬
곱을 돌리되, 값의 범위를 층마다 재어 정밀도를 맞추는 장치다. FP8은 16비트보다 데이터가 절반이라
같은 HBM 대역폭으로 두 배를 나르고 Tensor Core 처리량도 오른다. 이득은 조건이 맞아야 나온다. 행렬
곱이 큰 트랜스포머 계열에서 크고, 작은 모델이나 통신·데이터 로딩이 병목인 워크로드에서는 FP8로
바꿔도 스텝 시간이 줄지 않는다.

카탈로그 수치를 실제 성능으로 바로 옮기면 어긋난다. 표에 적힌 대역폭과 최대 연산량은 이상적인
조건의 상한이라, 실제로는 그 절반을 밑도는 경우가 흔하다. 최대 연산량은 대개 희소성(sparsity)을
켜고 특정 정밀도를 쓴 값이라, 그 조건이 아니면 절반으로 봐야 한다. HBM 대역폭도 접근 패턴이
연속적일 때의 값이라, 조각난 접근에서는 크게 못 미친다. 실제 성능은 돌리려는 모델로 스텝 시간과
달성 대역폭을 재서 잡는다.

```bash
# 카드 모델과 정밀도별 클럭, 메모리를 확인한다
nvidia-smi --query-gpu=name,compute_cap,memory.total,clocks.max.sm --format=csv
# 실제 달성 대역폭과 연산 효율은 프로파일러로 잰다
ncu --metrics dram__throughput.avg.pct_of_peak_sustained_elapsed ./kernel_test
```

```bash
# Tensor Core를 실제로 쓰는지 커널 이름과 정밀도로 확인한다
nsys profile -t cuda --stats=true python train.py 2>&1 | grep -iE "gemm|tensor|fp8"
```

컴퓨트 능력(compute capability) 값은 세대를 코드로 구분하는 번호다. A100은 8.0, H100·H200은 9.0이라,
프레임워크가 FP8 커널을 쓸지 결정하는 근거가 된다. 이 값을 확인하면 카드가 특정 정밀도를 하드웨어로
지원하는지 가릴 수 있다.
...
8장 노드는 GPU가 NVSwitch를 거쳐 서로 붙는다. 이 스위치의 링크가 죽으면 특정 GPU 쌍만이 아니라
여러 경로가 함께 느려진다. 앞의 NVLink 절이 카운터 읽는 법을 짚었다면, 이 절은 NVSwitch가 있는
노드에서 링크가 죽었을 때 로그에 남는 형태와 판단을 짚는다.

링크가 죽으면 드라이버가 커널 로그에 흔적을 남긴다. NVSwitch 관련 줄은 `SXid`라는 코드로 찍히는데,
GPU의 Xid에 해당하는 스위치판이다.

```bash
dmesg -T | grep -iE "nvswitch|SXid"
journalctl -k --since "1 hour ago" | grep -iE "nvswitch|fabric"
```

로그에 나오는 문구로 대략의 층을 가른다. 링크 마스크 갱신 실패(link mask update failed)나 링크가
올라오지 않았다는 줄은 그 링크가 협상 단계에서 붙지 못했다는 뜻이라, 물리 접속이나 커넥터를
의심한다. Rx 파이프라인 이벤트(Rx pipeline error) 같은 수신 경로 오류는 링크는 붙었으나 받는 쪽에서
오류가 난다는 뜻이라, 링크 품질이나 스위치 포트를 본다. 어느 쪽이든 SXid 뒤에 붙은 스위치와 포트
번호로 어느 링크인지 좁힌다.

오류 카운터는 시간에 따라 봐야 심각도가 갈린다. 절대 누적값은 부팅 이후 쌓인 것이라, 초기화한 뒤
증가 속도를 재는 것이 핵심이다.

```bash
nvidia-smi nvlink -e                     # 현재 오류 카운터
nvidia-smi nvlink -r 0                    # 카운터를 초기화한다
sleep 600
nvidia-smi nvlink -e                     # 10분 동안의 증가분
```

카운터 종류마다 뜻과 심각도가 다르다.

| 카운터 | 뜻 | 증가 속도로 본 판단 |
| --- | --- | --- |
| `replay` | 전송 실패로 재전송했다 | 이따금은 정상, 분 단위 급증이면 물리 계층 |
| `recovery` | 링크가 잠깐 끊겨 복구했다 | 반복되면 케이블·커넥터 |
| `CRC` | 오류 검출 부호 불일치 | 지속 증가면 링크 품질, 교체 후보 |

10분에 몇 개 수준이면 지켜보고, 분마다 수십에서 수백으로 늘면 물리 링크나 스위치 포트로 본다.
`recovery`와 `CRC`가 함께 오르면 링크가 붙었다 끊기기를 반복하는 상태라 더 나쁘다.

NVSwitch 노드는 fabric manager라는 데몬이 스위치와 링크를 초기화하고 관리한다. 이 서비스가 죽어
있으면 GPU가 서로 못 붙어 학습이 시작조차 안 되거나 링크가 반만 올라온다.

```bash
systemctl status nvidia-fabricmanager    # 데몬 상태
journalctl -u nvidia-fabricmanager --since today | grep -iE "error|degraded|failed"
nvidia-smi -q | grep -A10 "Fabric"       # GPU가 fabric에 붙은 상태
```

링크 하나가 빠지면 집합 통신 성능이 어떻게 떨어지는지는 통신 패턴에 달렸다. 모든 GPU가 고르게
주고받는 올리듀스에서는 가장 느린 경로가 전체를 잡아, 링크 하나가 죽어 그 GPU의 대역폭이 줄면
스텝 시간이 눈에 띄게 는다. 확인은 카드 안 진단이 아니라 실제 통신을 재서 한다.

```bash
# 노드 안 8장으로 올리듀스 대역폭을 재서 규격과 견준다
all_reduce_perf -b 8 -e 8G -f 2 -g 8
# 링크 상태와 대역폭을 함께 본다
nvidia-smi nvlink -s | grep -iE "Link|GB/s"
```

`all_reduce_perf`로 잰 버스 대역폭이 정상 노드보다 눈에 띄게 낮고, `nvidia-smi nvlink -s`에서 링크
수가 규격보다 적으면 죽은 링크가 통신을 누르는 것이다. 재시작으로 복구되는 경우와 교체가 필요한
경우는 이렇게 가른다. fabric manager를 재시작하거나 노드를 리셋해 링크가 규격대로 다시 올라오고
카운터가 더 안 늘면 일시적 문제라 반입한다. 리셋해도 링크가 반만 올라오거나, `CRC`가 리셋 뒤에도
계속 늘거나, 같은 포트가 반복해서 죽으면 스위치나 케이블 교체 대상으로 돌린다. 판단이 애매하면
노드를 드레인하고 `dcgmi diag -r 3`과 통신 벤치마크를 함께 돌려 근거를 모은다.
