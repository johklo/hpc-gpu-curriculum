...
GPU 메모리가 모자라 배치를 못 키우는 상황은 흔하다. 계산을 줄이지 않고 메모리만 줄이는 방법이
몇 가지 있고, 각각 다른 것을 내준다.

| 기법 | 무엇을 줄이나 | 대가 |
| --- | --- | --- |
| 혼합 정밀도 | 활성값과 기울기를 절반 크기로 | 수치 범위가 좁아진다 |
| 활성값 재계산 | 역전파에 쓸 중간값을 안 들고 있는다 | 계산이 20에서 30퍼센트 늘어난다 |
| 기울기 누적 | 한 번에 올리는 배치를 쪼갠다 | 스텝당 시간이 늘어난다 |
| ZeRO · FSDP | 옵티마이저 상태와 가중치를 랭크에 나눈다 | 통신이 늘어난다 |
| 오프로딩 | 안 쓰는 상태를 호스트 메모리로 내린다 | PCIe를 타므로 느리다 |

혼합 정밀도가 가장 먼저 쓰는 방법이다. 계산은 16비트로 하고 가중치 갱신만 32비트로 유지한다.
메모리가 줄고 텐서 코어를 쓰게 되어 속도까지 오르므로 대가가 거의 없다.

정밀도 형식마다 성질이 다르다. fp16은 소수 자릿수가 많지만 표현 범위가 좁아 기울기가 0으로
내려앉는 일이 생긴다. bf16은 자릿수를 줄이고 범위를 fp32와 같게 잡아 그 문제가 없다. 최신
GPU에서는 bf16을 쓰는 편이 안전하다.

| 형식 | 지수 비트 | 가수 비트 | 성질 |
| --- | ---: | ---: | --- |
| fp32 | 8 | 23 | 기준. 메모리를 가장 많이 쓴다 |
| fp16 | 5 | 10 | 범위가 좁다. 손실 스케일링이 필요하다 |
| bf16 | 8 | 7 | 범위는 fp32와 같다. 정밀도가 낮다 |
| fp8 | 4 또는 5 | 3 또는 2 | 더 빠르다. 층마다 스케일 관리가 필요하다 |

```python
import torch

scaler = torch.amp.GradScaler("cuda")          # fp16 을 쓸 때만 필요하다

for batch in loader:
    with torch.autocast("cuda", dtype=torch.bfloat16):
        loss = model(batch)                     # 계산은 16비트로
    scaler.scale(loss).backward()
    scaler.step(optimizer)                      # 갱신은 32비트 마스터 가중치로
    scaler.update()
    optimizer.zero_grad(set_to_none=True)
```

bf16을 쓰면 `GradScaler`가 필요 없다. fp16에서만 기울기가 0으로 내려앉는 것을 막으려고 손실에
큰 수를 곱했다 나누는 과정이 필요하다.

혼합 정밀도가 잘 도는지는 손실 곡선과 텐서 코어 사용률로 확인한다. 손실이 갑자기 `nan`이
되면 범위 문제이므로 bf16으로 바꾸거나 스케일을 조정한다. 속도가 안 오르면 텐서 코어를 안 쓰고
있을 수 있고, 행렬 크기가 8의 배수가 아니면 이런 일이 생긴다.

```bash
# 텐서 코어가 실제로 쓰이는지 커널 이름으로 확인한다
nsys profile -o mp --stats=true python train.py 2>&1 | grep -iE "s16816|hmma|bf16"
```

## 분산 학습 프레임워크 고르기

같은 데이터 병렬이라도 구현이 여럿이다. 무엇을 쓰든 통신은 NCCL이 하므로 성능 차이보다 쓰기
편한 정도와 기능 범위가 선택을 가른다.

| 도구 | 성격 | 맞는 자리 |
| --- | --- | --- |
| PyTorch DDP | 표준 내장. 데이터 병렬만 | 모델이 GPU 한 장에 들어갈 때 |
| PyTorch FSDP | 내장. 가중치와 옵티마이저를 랭크에 나눈다 | 큰 모델을 PyTorch만으로 |
| DeepSpeed | ZeRO 1·2·3, 오프로딩, 파이프라인 병렬 | 메모리가 빠듯한 대형 학습 |
| Megatron-LM | 텐서와 파이프라인 병렬 구현 | 수백억 파라미터 이상 |
| Horovod | 프레임워크 중립. MPI 방식으로 띄운다 | TensorFlow가 섞여 있을 때 |

DeepSpeed는 설정 파일로 단계를 고른다. 단계가 올라갈수록 메모리는 줄고 통신은 는다.
...
```bash
# Slurm 위에서 DeepSpeed 를 띄운다. 호스트 파일 대신 srun 이 프로세스를 배치한다
srun --ntasks-per-node=8 python -m deepspeed.launcher.launch \
     --no_ssh --node_rank=$SLURM_NODEID train.py --deepspeed ds_config.json
```

Horovod는 MPI로 프로세스를 띄우므로 Slurm과 잘 맞는다. 다만 PyTorch만 쓴다면 굳이 고를 이유가
줄었다. 오래된 코드를 이어받는 경우가 대부분이다.

| 확인할 것 | 그렇다면 |
| --- | --- |
| 모델이 GPU 한 장에 들어간다 | DDP로 충분하다 |
| 옵티마이저 상태가 안 들어간다 | FSDP나 DeepSpeed ZeRO 2 |
| 가중치도 안 들어간다 | ZeRO 3이나 텐서 병렬 |
| 층 하나가 GPU 한 장을 넘는다 | 텐서 병렬이 필요하다 |
| 이미 TensorFlow 코드가 있다 | Horovod |

## 추론을 같은 클러스터에서 돌릴 때

학습만 있는 클러스터는 드물다. 학습한 모델을 서비스하려면 성격이 다른 워크로드가 같은 자원을
쓰게 된다.

| 항목 | 학습 | 추론 |
| --- | --- | --- |
| 수명 | 몇 시간에서 몇 주 | 계속 살아 있다 |
| 목표 | 처리량 | 응답 지연 |
| 자원 형태 | GPU 여러 장을 한 덩어리로 | GPU 한 장 또는 그 일부 |
| 부하 | 일정하다 | 시간대에 따라 변한다 |
| 실패 | 재시작하고 이어간다 | 즉시 다른 복제본이 받는다 |
| 여유 | 채울수록 좋다 | 남겨 둬야 한다 |

목표가 반대라 같은 노드에 섞으면 둘 다 나빠진다. 학습이 GPU를 채우면 추론 지연이 튀고, 추론을
위해 여유를 남기면 학습 처리량이 떨어진다. 노드를 나누는 것이 기본이고, 나눈 뒤에 남는 문제는
추론 쪽 자원이 노는 것이다.

추론은 GPU 한 장을 통째로 쓰지 않는 경우가 많으므로 나눠 쓰는 방법을 검토한다. 격리가 필요하면
MIG로 하드웨어에서 나누고, 처리량이 목적이면 MPS나 서빙 엔진의 배치 기능을 쓴다.

```yaml
# MIG 인스턴스를 요청한다. 카드 한 장을 여럿이 나눠 쓴다
resources:
  limits:
    nvidia.com/mig-1g.10gb: 1
```

시간대별로 자원을 옮기는 정책이 통하려면 추론 부하가 실제로 시간을 탄다는 근거가 있어야 한다.
사내 서비스는 밤에 부하가 크게 주는 경우가 많아 그 시간대를 학습에 넘길 수 있다. 대외 서비스는
그렇지 않은 경우가 많으므로 옮기기 전에 지난 부하 곡선부터 본다.

```bash
# 지난 4주 시간대별 추론 GPU 사용률. 밤에 실제로 비는지 확인한다
cat <<'PROMQL'
avg by (hour) (
  avg_over_time(DCGM_FI_DEV_GPU_UTIL{pool="inference"}[4w])
)
PROMQL
```
