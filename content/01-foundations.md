...
이 모듈은 그 묶음이 어떤 부품으로 이루어지고, 사용자가 제출한 계산이 어느 경로로 실행되는지를
다룬다.
...
역할을 나누는 이유는 두 가지다. 노드 한 대가 죽어도 전체가 멈추지 않게 하는 것이 하나고,
비싼 GPU 노드를 계산에만 쓰게 하는 것이 다른 하나다.
...
사용자는 "GPU 4장, 12시간, 이 스크립트"라는 요청서를 제출하고 로그아웃한다. 스케줄러는 자원이
빌 때 요청을 꺼내 실행한다. 대화형 실행과 다른 점은 세 가지다.

- 제출과 실행이 분리된다. 새벽 3시에 시작해도 문제없다.
- 실행 중인 작업은 할당받은 GPU와 메모리를 독점한다. 옆 사람 작업이 내 메모리를 잠식하지 않는다.
- 우선순위, 사용량 한도, 공정 배분을 규칙으로 강제할 수 있다.

클러스터 운영의 상당 부분은 "누가 얼마나 오래 무엇을 쓰는가"라는 정책 문제로 귀결된다.
이 주제는 모듈 04에서 다룬다.

## Slurm의 구조

Slurm은 HPC에서 가장 널리 쓰는 오픈소스 스케줄러다. 데몬 세 개만 알면 전체 그림이 잡힌다.
...
개념은 두 개다. `rank`는 프로세스마다 붙는 0부터의 번호이고, `size`는 전체 프로세스 수다.
...
통신은 두 갈래다. 점대점 통신은 `send`와 `recv`로 특정 상대와 주고받고, 집합 통신은
